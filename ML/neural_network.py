...
def grad(H, W, y, landa):
    ...
    # print('theta')
    # for i, w0 in zip(range(len(W)), W):
    #     print(i, ':', w0.shape)
    # print()
    # print('H')
    # for i, h1 in zip(range(len(H)), H):
    #     print(i, ':', h1.shape)
    ...
    for h0, h1, w0, w1 in zip(H[:-2][::-1], H[1:-1][::-1], W[:-1][::-1], W[1:][::-1]):
        # print(delta[0].shape)
        # print(w1.T[1:, :].shape, delta[0].shape, h1.shape)
...
def fit(X, y, W, landa, max_iter, activation):
    for i in range(max_iter):
        # J.append(cost(X, y, W, landa, activation))
